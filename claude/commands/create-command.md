...
## BEFEHLSPOSITION BESTIMMEN

1. Platziere den Befehl im entsprechenden Kategorieverzeichnis:
   - `code-analysis-testing/` - Für Code-Analyse, Tests und Qualitätssicherung
   - `ci-deployment/` - Für CI/CD und Deployment-bezogene Befehle
   - `context-loading-priming/` - Für Kontext-Laden und Priming-Befehle
   - `documentation-changelogs/` - Für Dokumentations- und Changelog-Befehle
   - `project-task-management/` - Für Projekt- und Task-Management-Befehle
   - `version-control-git/` - Für Versionskontrolle und Git-Operationen
   - `miscellaneous/` - Für Befehle die in keine andere Kategorie passen
